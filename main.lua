local AshlyGUI = (function()
    ...
    function AshlyGUI:CreateWindow(config)
        ...
        MakeDraggable(container) -- allow dragging from body area too
                    ...
                    AddSection = function(self, sectionConfig)
                        ...
                        local secAPI = {
                            AddToggle = function(_, toggleConfig)
                                ...
                                frame.InputBegan:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 then Update() end end)
                            ...
                            AddSlider = function(_, sliderConfig)
                                ...
                                frame.InputBegan:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 then dragging = true; Update(i.Position.X) end end)
                            ...
                            AddDropdown = function(_, ddConfig)
                                ...
                                btn.MouseButton1Click:Connect(function()
                                    ...
                                    if expanded then
                                        ...
                                        -- Parent to section (not frame) so it escapes the dropdown frame's clipping bounds
                                        local absPos = btn.AbsolutePosition
                                        local absSize = btn.AbsoluteSize
                                        listFrame = CreateInstance("ScrollingFrame", {
                                            Size = UDim2.new(0, 110, 0, math.min(#options * 26, 130)),
                                            Position = UDim2.new(0, absPos.X - sec.AbsolutePosition.X, 0, absPos.Y - sec.AbsolutePosition.Y + absSize.Y + 2),
                                            BackgroundColor3 = Color3.fromRGB(25, 25, 25), BorderSizePixel = 0,
                                            ScrollBarThickness = 2, ZIndex = 20, Parent = sec
                                        })
                                        ...
                                        for _, opt in ipairs(options) do
                                            local optBtn = CreateInstance("TextButton", { Size = UDim2.new(1, 0, 0, 22), BackgroundColor3 = Color3.fromRGB(35, 35, 35), Text = opt, TextColor3 = Color3.fromRGB(180, 180, 180), TextSize = 11, Font = Enum.Font.Gotham, ZIndex = 21, BorderSizePixel = 0, Parent = listFrame })
...
local KEY_FILE = "BloxStrike_SavedKey.txt"

local CONFIG_FILE = "BloxStrike_Settings.json"
local LoadedConfig = { ToggleKey = "K", Toggles = {}, Sliders = {}, Dropdowns = {}, TotalPlayTime = 0 }
...
local function LoadFreeScript()
    ...
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "Ashly Hub",
            Text = "To open it again just click K",
            Duration = 5
        })
    end)

    local ToggleKeybind = Enum.KeyCode.K
    pcall(function() ToggleKeybind = Enum.KeyCode[LoadedConfig.ToggleKey] or Enum.KeyCode.K end)
    ...
    ESPSec:AddToggle({
        Name = "Chams",
        Default = getToggleVal("Chams", false),
        Callback = function(Value)
            AshlyState.ChamsEnabled = Value
            if LoadedConfig.Toggles then LoadedConfig.Toggles["Chams"] = Value; SaveConfig() end
        end
    })

    -- ── Aimbot Tab ──
    local Tab2 = Window:CreateTab({ Name = "Aimbot", Icon = "🎯" })
    local AimbotSec = Tab2:AddSection({ Name = "Aimbot Settings" })
    ...
    AimbotSec:AddToggle({
        ...
        Default = getToggleVal("Auto-Shoot (TriggerBot)", false),
        Callback = function(Value)
            AshlyState.AutoShootEnabled = Value
            if LoadedConfig.Toggles then LoadedConfig.Toggles["Auto-Shoot (TriggerBot)"] = Value; SaveConfig() end
        end
    })

    AimbotSec:AddLabel({ Text = "Hold Left Alt to Aimbot", Color = Color3.fromRGB(150, 150, 150) })
    ...
    -- Visuals Tab Removed by User Request
    ...
    RunService:BindToRenderStep("AshlyAimbot", 201, function()
        ...
        -- Only move mouse if distance is significant to avoid input dropping (single-fire bug fix)
        if math.abs(diffX) > 1 or math.abs(diffY) > 1 then
            if mousemoverel then
                pcall(mousemoverel, diffX / smooth, diffY / smooth)
            else
                local targetCFrame = CFrame.new(Camera.CFrame.Position, targetPos)
                Camera.CFrame = Camera.CFrame:Lerp(targetCFrame, math.clamp(1 / smooth, 0.05, 1))
            end
        end

        if AshlyState.AutoShootEnabled then
            local screenDelta = (Vector2.new(pos.X, pos.Y) - mouseLoc).Magnitude
            if screenDelta < 20 then
                local now = tick()
    ...
    local AvailableSkinRemotes = {}
    local AllSkinIds = {}

    local function DiscoverAllSkins()
        pcall(function()
            ...
            for _, c in ipairs(containers) do
                for _, obj in ipairs(c:GetDescendants()) do
                    if obj:IsA("RemoteEvent") or obj:IsA("RemoteFunction") then
                        ...
                        if n:find("skin") or n:find("cosmetic") or n:find("equip")
                           or n:find("outfit") or n:find("select") or n:find("loadout")
                           or n:find("appearance") then
                            table.insert(AvailableSkinRemotes, obj)
                        end
                    end
                    if obj:IsA("NumberValue") or obj:IsA("IntValue") or obj:IsA("StringValue") then
                        local n = obj.Name:lower()
                        if n:find("skin") or n:find("id") then
                            table.insert(AllSkinIds, { Name = obj.Name, Value = obj.Value, Object = obj })
                        end
                    end
                end
            end
    ...
    pcall(function()
        ...
        if pg then
            for _, child in ipairs(pg:GetChildren()) do
                ...
                if ok and env then ClientEnv = env; break end
            end
        end
    end)

    local function GetAllWeaponInstances()
        local instances = {}
        local char = LocalPlayer.Character
        if char then
            for _, tool in ipairs(char:GetChildren()) do
                if tool:IsA("Tool") then
                    for _, part in ipairs(tool:GetDescendants()) do instances[part] = true end
                end
            end
            local arms = char:FindFirstChild("Arms")
            if arms then
                for _, part in ipairs(arms:GetDescendants()) do instances[part] = true end
            end
        end
        local cam = workspace.CurrentCamera
        if cam then
            local viewModel = cam:FindFirstChild("ViewModel") or cam:FindFirstChild("Arms")
            if viewModel then
                for _, part in ipairs(viewModel:GetDescendants()) do instances[part] = true end
            end
            for _, child in ipairs(cam:GetChildren()) do
                if child:IsA("Model") or child:IsA("Tool") then
                    for _, part in ipairs(child:GetDescendants()) do instances[part] = true end
                end
            end
        end
        for _, obj in ipairs(workspace:GetChildren()) do
            if obj:IsA("Model") then
                ...
                if nl:find("weapon") or nl:find("gun") or nl:find("tool") then
                    for _, part in ipairs(obj:GetDescendants()) do instances[part] = true end
                end
            end
        end
        return instances
    ...
    local function ApplySkinToInstances(instances, skinName)
        if not AshlyState.SkinUnlockEnabled then return end
        local preset = SkinPresets and SkinPresets[skinName]
        if not preset then return end
        local skinColor, skinMat = preset[1], preset[2]
        pcall(function()
            for part in pairs(instances) do
                if part:IsA("BasePart") then
                    pcall(function() part.Color = skinColor end)
                    pcall(function() part.Material = skinMat end)
                ...
                if part:IsA("IntValue") or part:IsA("NumberValue") or part:IsA("StringValue") then
                    local pn = part.Name:lower()
                    if pn:find("skin") or pn:find("cosmetic") or pn:find("id") then
                        pcall(function()
                            if part:IsA("StringValue") then part.Value = skinName
                            else part.Value = AshlyState.SelectedSkinId or 1 end
                        end)
                    end
                end
            end
    ...
    local function GetCurrentWeaponCategory()
        if AshlyState.SelectedWeaponCategory then return AshlyState.SelectedWeaponCategory end
        if SkinData then
            for weapon, skins in pairs(SkinData) do
                for _, s in ipairs(skins) do
                    if s == AshlyState.SelectedSkinName then return weapon end
                end
            end
        end
    ...
    local oldIndexSkin
    pcall(function()
        oldIndexSkin = hookmetamethod(game, "__index", function(self, key)
            if AshlyState.SkinUnlockEnabled and type(key) == "string" then
                local kl = key:lower()
                if kl == "owned" or kl == "unlocked" or kl == "hasskin" or kl == "hascosmetic" then
                    local ok, selfName = pcall(function() return tostring(self):lower() end)
                    if ok and (selfName:find("skin") or selfName:find("cosmetic")) then
                        return true
                    end
                end
            end
            return oldIndexSkin(self, key)
        end)
    end)

    getfenv().ApplySelectedSkinNow = function()
        if not AshlyState.SkinUnlockEnabled then return end
        local skinName = AshlyState.SelectedSkinName
        if not skinName then return end

        local skinId = 1
        for _, info in ipairs(AllSkinIds) do
            if info.Name:lower():find(skinName:lower()) then
                skinId = type(info.Value) == "number" and info.Value or 1
                break
            end
        end
        AshlyState.SelectedSkinId = skinId

        local allInstances = GetAllWeaponInstances()
        ApplySkinToInstances(allInstances, skinName)

        for _, remote in ipairs(AvailableSkinRemotes) do
            pcall(function()
                local payload = {
                    Skin = skinName, SkinId = skinId,
                    Weapon = GetCurrentWeaponCategory(),
                    Owned = true, Unlocked = true, HasSkin = true
                }
        ...
        if ClientEnv then
            pcall(function()
                if ClientEnv.EquipSkin then ClientEnv.EquipSkin(skinName) end
                if ClientEnv.SelectSkin then ClientEnv.SelectSkin(skinName, skinId) end
                if ClientEnv.ApplySkin then ClientEnv.ApplySkin(GetCurrentWeaponCategory(), skinName) end
            end)
        end
                        ...
                        if child:IsA("RemoteEvent") or child:IsA("RemoteFunction") then
                            pcall(function()
                                if child:IsA("RemoteEvent") then
                                    child:FireServer({ Skin = skinName, Owned = true })
                                else
                                    child:InvokeServer({ Skin = skinName, Owned = true })
                                end
                            end)
                        end
                    end
    ...
    local function SetupWeaponSkinHooks(character)
        if not character then return end
        task.spawn(getfenv().ApplySelectedSkinNow)
        character.ChildAdded:Connect(function(child)
            if child:IsA("Tool") and AshlyState.SkinUnlockEnabled then
                task.delay(0.15, getfenv().ApplySelectedSkinNow)
            end
        end)
        local cam = workspace.CurrentCamera
        if cam then
            cam.ChildAdded:Connect(function(child)
                if AshlyState.SkinUnlockEnabled then
                    task.delay(0.1, getfenv().ApplySelectedSkinNow)
                end
            end)
        end
    end

    local function OnCharacterAdded_Skin(character)
        task.wait(0.8)
        SetupWeaponSkinHooks(character)
        if AshlyState.SkinUnlockEnabled then
            task.delay(0.5, getfenv().ApplySelectedSkinNow)
        end
    end

    -- SKIN CHANGER LOGIC REMOVED

    local function HookCameraViewModel(cam)
        if not cam then return end
        cam.ChildAdded:Connect(function(child)
            if AshlyState.SkinUnlockEnabled and (child.Name == "ViewModel" or child.Name == "Arms") then
                task.delay(0.2, getfenv().ApplySelectedSkinNow)
            end
        end)
    end
    HookCameraViewModel(workspace.CurrentCamera)
    ...
    oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
        ...
        if AshlyState.SkinUnlockEnabled and (method == "FireServer" or method == "InvokeServer") then
            if self:IsA("RemoteEvent") or self:IsA("RemoteFunction") then
                local nameLow = self.Name:lower()
                if nameLow:find("skin") or nameLow:find("equip") or nameLow:find("cosmetic")
                   or nameLow:find("outfit") or nameLow:find("appearance") or nameLow:find("loadout")
                   or nameLow:find("select") or nameLow:find("unlock") then
